...
A complete implementation of a Merkle tree where elements are encrypted using Fully Homomorphic Encryption (FHE). This project explores zero-knowledge membership queries on encrypted data without revealing sensitive information.

## Project Overview

### Problem Statement
Traditional Merkle trees use hash functions to create a tree structure for efficient membership proofs. However, when data is encrypted using probabilistic HE schemes, each encryption of the same plaintext yields a different ciphertext. This property breaks the fundamental assumption that identical plaintexts produce identical leaves in a Merkle tree.
